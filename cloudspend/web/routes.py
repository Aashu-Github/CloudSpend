...
def _resource_payload(resource) -> dict:
    ...
    _cost, basis = monthly_equivalent(resource.costs)
    payload["cost_basis"] = basis
    return payload


def _scan_payload(scan: dict) -> dict:
    metrics = _dashboard_values(scan)
    resources = [_resource_payload(resource) for resource in scan["resources"]]
    recommendations = [rec.model_dump(mode="json") for rec in scan["recommendations"]]
    findings_by_resource: dict[str, int] = {}
    for rec in scan["recommendations"]:
        findings_by_resource[rec.resource_id] = findings_by_resource.get(rec.resource_id, 0) + 1
    for resource in resources:
        resource["finding_count"] = findings_by_resource.get(resource["resource_id"], 0)
    return {
        "scan_id": scan["id"],
        ...
        "summary": {
            "observed": float(metrics["observed"]),
            "savings": float(metrics["savings"]),
            "savings_pct": metrics["savings_pct"],
            "resource_count": len(scan["resources"]),
            "opportunity_count": len(scan["recommendations"]),
            "cost_basis_counts": metrics["cost_basis_counts"],
            "scan_info": metrics["scan_info"],
...
def _import_uploaded_file(upload) -> tuple[dict, int]:
    if upload is None or not upload.filename:
        return {"error": "Choose a file to import."}, 400
    try:
        ...
        return {"error": "Import failed safely. Verify that the file matches a supported AWS/CloudSpend schema."}, 400
...
def api_import():
    _require_csrf()
    payload, status = _import_uploaded_file(request.files.get("file"))
    if status == 200:
        payload["redirect"] = url_for("web.scan_dashboard", scan_id=payload["scan_id"])
...
@bp.get("/health")
def health():
    return jsonify({"status": "ok", "service": "cloudspend-api"})


@bp.get("/api/public/health")
def public_health():
    return jsonify({"status": "ok", "service": "cloudspend-api", "mode": "public-api"})


@bp.post("/api/public/demo")
def public_demo():
    """Stateless browser entry point for the GitHub Pages demo frontend."""
    sample = Path(current_app.root_path).parent.parent / "samples" / "mock_aws_bundle.zip"
    try:
        scan_id = _save(FixtureProvider(sample, _settings()).load(), "demo")
        return jsonify({"scan_id": scan_id})
    except Exception as exc:
        current_app.logger.warning("Public demo failed: %s", type(exc).__name__)
        return jsonify({"error": "The demo could not be loaded. Please try again or run CloudSpend locally."}), 500


@bp.post("/api/public/import")
def public_import():
    """Cross-origin file import for the static GitHub Pages frontend.

    This endpoint intentionally does not use the session CSRF token because the
    static frontend is on a different origin and no authenticated user state is
    modified. Upload validation, size limits, schema validation, and temp-file
    deletion are identical to local mode.
    """
    payload, status = _import_uploaded_file(request.files.get("file"))
    return jsonify(payload), status


@bp.get("/api/public/scans/<scan_id>")
def public_scan(scan_id: str):
    scan = _store().get_scan(scan_id)
    if not scan:
        return jsonify({"error": "Scan not found. Hosted demo scans are temporary and may expire after a backend restart."}), 404
    return jsonify(_scan_payload(scan))


@bp.get("/api/public/scans/<scan_id>/resources/<path:resource_id>")
def public_resource(scan_id: str, resource_id: str):
    found = _store().get_resource(scan_id, resource_id)
    if not found:
        return jsonify({"error": "Resource not found for this scan."}), 404
    resource, recs = found
    return jsonify({
        "scan_id": scan_id,
        "resource": _resource_payload(resource),
        "recommendations": [rec.model_dump(mode="json") for rec in recs],
    })
